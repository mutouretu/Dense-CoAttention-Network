...
from constants import BASE_DIR, DATA20, WORK_DIR, NLTK_DIR
...
data_map_vqa = {
    ...
    "train_comp_path": DATA20 + "v2_mscoco_train2014_complementary_pairs.json",
    "val_comp_path": DATA20 + "v2_mscoco_val2014_complementary_pairs.json",
}
...
def process_text(examples, without_ans=False, nlp="nltk"):
    ...
    for i, ex in enumerate(examples):
        ...
        if (i + 1) % 10000 == 0:
            sys.stdout.write(
                "processing %d/%d (%.2f%% done)	\r" % ((i + 1), len(examples), (i + 1) * 100.0 / len(examples)))
            sys.stdout.flush()
...
def process_dataset(dataset, num_occurs, glove_path, max_ques, max_ans):
    ...
    if True:
        ...
        dataset, max_len_ques, max_len_ans = build_glove_train(dataset, gloves[1])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default=WORK_DIR)
    parser.add_argument("--data_name", type=str, default="cocotrain")
    parser.add_argument("--max_ques", type=int, default=14)
    parser.add_argument("--max_ans", type=int, default=None)
    parser.add_argument("--glove_path", type=str, default=WORK_DIR+"glove_840B.pt")
